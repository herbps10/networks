...
for(i in seq(0, 400, by=1)) {
  ...
  if(i < 10) {
    ...
    file = paste("stats-0.", i, "000", sep="")
  }
  
  data = read.table(paste("/home/herb/git/networks/data/dataset-11/", file, ".csv", sep=""), header=T, sep=",")
  ...
  p = p + geom_tile(aes(fill=day), color="white") + scale_fill_gradient(low="white", high="black")
  #p = p + opts(title = i)
  print(p)
}
